...
_M0DTPC15error5Error40localreview_2fwfc_2eSolveError_2eInvalid.prototype.$tag = 1;
...
function _M0TP211localreview3wfc6Search(param0, param1, param2, param3, param4, param5, param6, param7, param8) {
  this.model = param0;
  this.weights = param1;
  this.periodic = param2;
  this.width = param3;
  this.height = param4;
  this.seed = param5;
  this.budget = param6;
  this.decisions = param7;
  this.backtracks = param8;
}
...
function _M0IP211localreview3wfc10SolveErrorPC15debug5Debug8to__reprGRP211localreview3wfc10SolveErrorE(_x_157) {
  let _arg_158;
  _L: {
...
function _M0MP211localreview3wfc6Search6random(self, n) {
  ...
  return (x.val >>> 0) % (n >>> 0) | 0;
}
function _M0FP211localreview3wfc4bits(mask) {
  const out = [];
  const _bind = 0;
  const _bind$2 = 30;
  let _tmp = _bind;
  while (true) {
    ...
    if (i < _bind$2) {
      if ((mask & 1 << i) !== 0) {
        _M0MPC15array5Array4pushGiE(out, i);
      }
      _tmp = i + 1 | 0;
      continue;
    } else {
...
function _M0MP211localreview3wfc6Search8neighbor(self, cell, dir) {
  const _tmp = self.width;
  if (_tmp === 0) {
    $panic();
  }
  const x = cell % _tmp | 0;
  const _tmp$2 = self.width;
  if (_tmp$2 === 0) {
    $panic();
  }
  const y = cell / _tmp$2 | 0;
  if (self.periodic) {
    let _tmp$3;
    switch (dir) {
      case 0: {
        const _tmp$4 = Math.imul(y, self.width) | 0;
        const _tmp$5 = self.width;
        if (_tmp$5 === 0) {
          $panic();
        }
        _tmp$3 = _tmp$4 + ((x + 1 | 0) % _tmp$5 | 0) | 0;
        break;
      }
      case 1: {
        const _tmp$6 = self.height;
        if (_tmp$6 === 0) {
          $panic();
        }
        _tmp$3 = (Math.imul((y + 1 | 0) % _tmp$6 | 0, self.width) | 0) + x | 0;
        break;
      }
      case 2: {
        const _tmp$7 = Math.imul(y, self.width) | 0;
        const _tmp$8 = self.width;
        if (_tmp$8 === 0) {
          $panic();
        }
        _tmp$3 = _tmp$7 + (((x + self.width | 0) - 1 | 0) % _tmp$8 | 0) | 0;
        break;
      }
      default: {
        const _tmp$9 = self.height;
        if (_tmp$9 === 0) {
          $panic();
        }
        _tmp$3 = (Math.imul(((y + self.height | 0) - 1 | 0) % _tmp$9 | 0, self.width) | 0) + x | 0;
      }
    }
    return _tmp$3;
  }
  switch (dir) {
    case 0: {
      return (x + 1 | 0) < self.width ? cell + 1 | 0 : undefined;
    }
    case 1: {
      return (y + 1 | 0) < self.height ? cell + self.width | 0 : undefined;
    }
    case 2: {
      return x > 0 ? cell - 1 | 0 : undefined;
    }
    default: {
      return y > 0 ? cell - self.width | 0 : undefined;
    }
  }
}
function _M0MP211localreview3wfc6Search9propagate(self, domains) {
  const queue = _M0MPC15array5Array5makeiGiE(domains.length, (i) => i);
  const head = new _M0TPB8MutLocalGiE(0);
  while (true) {
    if (head.val < queue.length) {
      self.budget = self.budget - 1 | 0;
      if (self.budget < 0) {
        return new _M0DTPC16result6ResultGbRP211localreview3wfc10SolveErrorE3Err(new _M0DTPC15error5Error40localreview_2fwfc_2eSolveError_2eInvalid("search budget exhausted"));
      }
      const cell = _M0MPC15array5Array2atGiE(queue, head.val);
      head.val = head.val + 1 | 0;
      if (_M0MPC15array5Array2atGiE(domains, cell) === 0) {
      ...
      while (true) {
        ...
        if (dir < _bind$2) {
          ...
          _L: {
            _L$2: {
              const _bind$3 = _M0MP211localreview3wfc6Search8neighbor(self, cell, dir);
            ...
            const support = new _M0TPB8MutLocalGiE(0);
            const _bind$3 = _M0FP211localreview3wfc4bits(_M0MPC15array5Array2atGiE(domains, cell));
            const _bind$4 = _bind$3.length;
            let _tmp$2 = 0;
            while (true) {
              const _ = _tmp$2;
              if (_ < _bind$4) {
                const tile = _bind$3[_];
                support.val = support.val | _M0MPC15array5Array2atGiE(_M0MPC15array5Array2atGRPB5ArrayGcEE(self.model.allowed, tile), dir);
                _tmp$2 = _ + 1 | 0;
                continue;
              } else {
                break;
            ...
            const next = _M0MPC15array5Array2atGiE(domains, other) & support.val;
            if (next === 0) {
              return new _M0DTPC16result6ResultGbRP211localreview3wfc10SolveErrorE2Ok(false);
            }
            if (next !== _M0MPC15array5Array2atGiE(domains, other)) {
              _M0MPC15array5Array3setGiE(domains, other, next);
...
function _M0MP211localreview3wfc6Search6search(self, domains) {
  const _bind = _M0MP211localreview3wfc6Search9propagate(self, domains);
  let _tmp;
  if (_bind.$tag === 1) {
    const _ok = _bind;
    _tmp = _ok._0;
  ...
  const minimum = new _M0TPB8MutLocalGdE(1e+100);
  const ties = new _M0TPB8MutLocalGiE(0);
  const _bind$2 = 0;
  const _bind$3 = domains.length;
  let _tmp$2 = _bind$2;
  while (true) {
    const i = _tmp$2;
    if (i < _bind$3) {
      const count = _M0FP211localreview3wfc4bits(_M0MPC15array5Array2atGiE(domains, i)).length;
      const sum = new _M0TPB8MutLocalGdE(0);
      const weighted_log = new _M0TPB8MutLocalGdE(0);
      const _bind$4 = _M0FP211localreview3wfc4bits(_M0MPC15array5Array2atGiE(domains, i));
      const _bind$5 = _bind$4.length;
      let _tmp$3 = 0;
      while (true) {
        const _ = _tmp$3;
        if (_ < _bind$5) {
          const tile = _bind$4[_];
          const weight = _M0MPC15array5Array2atGiE(self.weights, tile) + 0;
          sum.val = sum.val + weight;
          weighted_log.val = weighted_log.val + weight * _M0FPC14math2ln(weight);
          _tmp$3 = _ + 1 | 0;
          continue;
        } else {
          break;
      ...
      const entropy = _M0FPC14math2ln(sum.val) - weighted_log.val / sum.val;
      if (count > 1 && entropy < minimum.val) {
        selected.val = i;
        minimum.val = entropy;
        ties.val = 1;
      } else {
        if (count > 1 && entropy === minimum.val) {
          ties.val = ties.val + 1 | 0;
          if (_M0MP211localreview3wfc6Search6random(self, ties.val) === 0) {
            selected.val = i;
          }
        }
      }
  ...
  if (selected.val < 0) {
    return new _M0DTPC16result6ResultGORPB5ArrayGiERP211localreview3wfc10SolveErrorE2Ok(new _M0DTPC16option6OptionGRPB5ArrayGiEE4Some(_M0MPC15array5Array3mapGiiE(domains, (d) => _M0MPC15array5Array2atGiE(_M0FP211localreview3wfc4bits(d), 0))));
  }
  self.decisions = self.decisions + 1 | 0;
  const choices = _M0FP211localreview3wfc4bits(_M0MPC15array5Array2atGiE(domains, selected.val));
  const _bind$4 = 0;
  const _bind$5 = choices.length;
  let _tmp$3 = _bind$4;
  while (true) {
    const i = _tmp$3;
    if (i < _bind$5) {
      const total = new _M0TPB8MutLocalGiE(0);
      const _bind$6 = choices.length;
      let _tmp$4 = i;
      while (true) {
        const k = _tmp$4;
        if (k < _bind$6) {
          total.val = total.val + _M0MPC15array5Array2atGiE(self.weights, _M0MPC15array5Array2atGiE(choices, k)) | 0;
          _tmp$4 = k + 1 | 0;
          continue;
        } else {
          break;
      ...
      const draw = new _M0TPB8MutLocalGiE(_M0MP211localreview3wfc6Search6random(self, total.val));
      const j = new _M0TPB8MutLocalGiE(i);
      while (true) {
        if ((j.val + 1 | 0) < choices.length && draw.val >= _M0MPC15array5Array2atGiE(self.weights, _M0MPC15array5Array2atGiE(choices, j.val))) {
          draw.val = draw.val - _M0MPC15array5Array2atGiE(self.weights, _M0MPC15array5Array2atGiE(choices, j.val)) | 0;
          j.val = j.val + 1 | 0;
          continue;
        } else {
          break;
      ...
      const temp = _M0MPC15array5Array2atGiE(choices, i);
      _M0MPC15array5Array3setGiE(choices, i, _M0MPC15array5Array2atGiE(choices, j.val));
      _M0MPC15array5Array3setGiE(choices, j.val, temp);
      const branch = _M0MPC15array5Array4copyGiE(domains);
      _M0MPC15array5Array3setGiE(branch, selected.val, 1 << _M0MPC15array5Array2atGiE(choices, i));
      let solution;
      _L: {
        _L$2: {
          const _bind$7 = _M0MP211localreview3wfc6Search6search(self, branch);
          let _bind$8;
          if (_bind$7.$tag === 1) {
            const _ok = _bind$7;
            _bind$8 = _ok._0;
          ...
          if (_bind$8.$tag === 1) {
            const _Some = _bind$8;
            const _solution = _Some._0;
            solution = _solution;
            break _L$2;
          }
          break _L;
        }
        return new _M0DTPC16result6ResultGORPB5ArrayGiERP211localreview3wfc10SolveErrorE2Ok(new _M0DTPC16option6OptionGRPB5ArrayGiEE4Some(solution));
      }
...
function _M0FP211localreview3wfc13solve_2einner(model, width, height, seed, pins, budget, weights, periodic) {
  ...
  if (width < 1 || (height < 1 || (width > 256 || (height > 256 || ((Math.imul(width, height) | 0) > 256 || (count < 1 || (count > 30 || (model.allowed.length !== count || budget < 1)))))))) {
    return new _M0DTPC16result6ResultGORP211localreview3wfc8SolutionRP211localreview3wfc10SolveErrorE3Err(new _M0DTPC15error5Error40localreview_2fwfc_2eSolveError_2eInvalid("invalid model dimensions or budget"));
  }
  const weights$2 = _M0MPC15array5Array9is__emptyGiE(weights) ? _M0MPC15array5Array4makeGiE(count, 1) : _M0MPC15array5Array4copyGiE(weights);
  if (weights$2.length !== count || _M0MPB4Iter3anyGiE(_M0MPC15array5Array4iterGiE(weights$2), (w) => w < 1 || w > 1000000)) {
    return new _M0DTPC16result6ResultGORP211localreview3wfc8SolutionRP211localreview3wfc10SolveErrorE3Err(new _M0DTPC15error5Error40localreview_2fwfc_2eSolveError_2eInvalid("one positive weight per tile, maximum 1000000"));
  }
  const full = (1 << count) - 1 | 0;
  const _bind = model.allowed;
  ...
  while (true) {
    ...
    if (_ < _bind$2) {
      const row = _bind[_];
      if (row.length !== 4) {
        return new _M0DTPC16result6ResultGORP211localreview3wfc8SolutionRP211localreview3wfc10SolveErrorE3Err(new _M0DTPC15error5Error40localreview_2fwfc_2eSolveError_2eInvalid("each tile requires four directions"));
      }
      ...
      while (true) {
        ...
        if (_$2 < _bind$3) {
          ...
          if (mask < 0 || (mask & full) !== mask) {
            return new _M0DTPC16result6ResultGORP211localreview3wfc8SolutionRP211localreview3wfc10SolveErrorE3Err(new _M0DTPC15error5Error40localreview_2fwfc_2eSolveError_2eInvalid("invalid tile mask"));
          }
          _tmp$2 = _$2 + 1 | 0;
          continue;
        } else {
      ...
      while (true) {
        ...
        if (dir < _bind$5) {
          ...
          while (true) {
            const other = _tmp$4;
            if (other < count) {
              const _tmp$5 = _M0MPC15array5Array2atGiE(_M0MPC15array5Array2atGRPB5ArrayGcEE(model.allowed, t), dir) >> other & 1;
              const _tmp$6 = _M0MPC15array5Array2atGRPB5ArrayGcEE(model.allowed, other);
              if (4 === 0) {
                $panic();
              }
              if (_tmp$5 !== (_M0MPC15array5Array2atGiE(_tmp$6, (dir + 2 | 0) % 4 | 0) >> t & 1)) {
                return new _M0DTPC16result6ResultGORP211localreview3wfc8SolutionRP211localreview3wfc10SolveErrorE3Err(new _M0DTPC15error5Error40localreview_2fwfc_2eSolveError_2eInvalid("asymmetric adjacency"));
              }
              _tmp$4 = other + 1 | 0;
              continue;
            } else {
  ...
  const domains = _M0MPC15array5Array4makeGiE(Math.imul(width, height) | 0, full);
  const _bind$4 = pins.length;
  let _tmp$3 = 0;
  while (true) {
    const _ = _tmp$3;
    if (_ < _bind$4) {
      const pin = pins[_];
      if (pin._0 < 0 || (pin._0 >= domains.length || (pin._1 < 0 || pin._1 >= count))) {
      ...
      _M0MPC15array5Array3setGiE(domains, pin._0, _M0MPC15array5Array2atGiE(domains, pin._0) & 1 << pin._1);
      _tmp$3 = _ + 1 | 0;
      continue;
    } else {
      break;
  ...
  const state = new _M0TP211localreview3wfc6Search(model, weights$2, periodic, width, height, seed === 0 ? 1 : seed, budget, 0, 0);
  let tiles;
  _L: {
    const _bind$5 = _M0MP211localreview3wfc6Search6search(state, domains);
    let _bind$6;
    if (_bind$5.$tag === 1) {
      const _ok = _bind$5;
      _bind$6 = _ok._0;
    ...
    if (_bind$6.$tag === 0) {
      ...
      const _Some = _bind$6;
      const _tiles = _Some._0;
      tiles = _tiles;
      break _L;
    }
  }
  return new _M0DTPC16result6ResultGORP211localreview3wfc8SolutionRP211localreview3wfc10SolveErrorE2Ok(new _M0TP211localreview3wfc8Solution(width, height, tiles, state.decisions, state.backtracks));
}
...
function _M0MP211localreview3wfc8Solution6render(self, model) {
  if (self.width < 1 || (self.height < 1 || (self.width > 256 || (self.height > 256 || self.tiles.length !== (Math.imul(self.width, self.height) | 0))))) {
            ...
            if (turn < _bind$4) {
              ...
              while (true) {
                ...
                if (mirror < _bind$6) {
                  ...
                  if (found.val >= 0) {
                    const _array_1 = weights;
                    const _index_2 = found.val;
...
export { _M0FP411localreview3wfc3cmd3web7overlap as overlap, _M0FP411localreview3wfc3cmd3web3run as run }
